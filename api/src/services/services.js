...
const dbResultMaper = async (respuesta) => {
  return await respuesta.map(e => {
    return {
      id: e.id,
...
const apiResultMaper = async (respuesta) => {
  return await respuesta.map(e => {
    return {
      id: e.id,
      healthScore: e.healthScore,
...
const apiWordSearch = async (param) => {
  if (!param) {
