...
export default function PageContainer() {
  ...
  const [currentPage, setCurrentPage] = useState(1); //página actual siempre es la 1
  ...
  const currentRecipes = RecipesFilter.length > 0 && RecipesFilter.slice(indexOfFirstRecipe, indexOfLastRecipe) //recetas de la página actual (9, 18, 27, 36, 45, 54, 63, 72, 81)
  const paginate = (pageNumber) => setCurrentPage(pageNumber); //función para cambiar de página y actualizar el state de currentPage y currentRecipes
